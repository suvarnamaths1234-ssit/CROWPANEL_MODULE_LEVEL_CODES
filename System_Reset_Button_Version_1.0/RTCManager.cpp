...
void RTCManager::buttonCallback(lv_event_t *e) {
    if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
        ...
        if (instance) {
            ...
            if (!instance->rtcReady) {
                Serial.println("WARNING: RTC hardware not connected");
            }
        } else {
            Serial.println("ERROR: RTCManager instance is NULL!");
        }
    }
}
...
void RTCManager::showCalendarPopup() {
    ...
    // Allow popup to open even if RTC not ready
     //if (!rtcReady) {
