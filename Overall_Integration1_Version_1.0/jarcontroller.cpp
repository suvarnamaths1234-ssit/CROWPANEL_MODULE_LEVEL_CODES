...
sprintf(data, "Jar%d,RPM:%d,Duration:%d\n", 
 ...
 // NEW - with timestamp
char data[256];  // Increased size for timestamp

// Get current time from RTC
DateTime now = rtcManager.getCurrentTime();

// Format: JarID,RPM,Duration,Date,Time
/*sprintf(data, "Jar%d,RPM%d,Duration%d,%04d-%02d-%02d,%02d:%02d:%02d\n", 
        jarID, setRPM, setDuration,
        now.year(),now.month(), now.day(),
        now.hour(), now.minute(), now.second());*/

/*sprintf(data, "Jar%d,%04d-%02d-%02d %02d:%02d:%02d,RPM%d,Duration%d\n", 
        jarID,now.year(),now.month(), now.day(),
        now.hour(), now.minute(), now.second(), setRPM, setDuration); 
          //Serial.printf("Jar %d,%d,%d\n");   
...
bool JarController::saveSettings(const char *filepath) {
    ...
    // ===== SERIAL COMMUNICATION FORMAT (for Master-Slave) =====
    // Simple format without timestamp: Jar1,15,20
    char serialData[64];
    sprintf(serialData, "motor%d,%d,%d\n", jarID, setRPM, setDuration);
    Serial.print(serialData);  // Send to slave jars
    
    
    // ===== SD CARD FORMAT (with timestamp) =====
    char sdData[256];
    DateTime now = rtcManager.getCurrentTime();
    
    // Format: Jar1,2025-12-05 14:30:00,RPM15,Duration20
    sprintf(sdData, "Jar%d,%04d-%02d-%02d %02d:%02d:%02d,RPM%d,Duration%d\n", 
            jarID,
            now.year(), now.month(), now.day(),
            now.hour(), now.minute(), now.second(),
            setRPM, setDuration);
    
    //Serial.printf("Jar %d: Attempting to write to SD: %s", jarID, sdData);
    ...
    size_t written = file.print(sdData);  // Use sdData (with timestamp)
   Serial.printf("Jar %d: Bytes written: %d\n", jarID, (int)written);
