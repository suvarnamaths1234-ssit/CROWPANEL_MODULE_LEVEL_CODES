...
// Global buffer for UART input
char command[64];
int idx = 0;
RTC_DS3231 rtc;

// ===== TIMESTAMP FILTER VARIABLES =====//timestamp
String filterStartTS = "";
String filterEndTS = "";
bool filterInputMode = false;
bool isFilteredView = false;  // NEW: Track if we're showing filtered data
enum FilterInputState { FILTER_START, FILTER_END, FILTER_COMPLETE };
FilterInputState filterState = FILTER_START;
...
void showSdLogOnSettings(lv_event_t *e) {
    ...
    // ===== ADD THIS CHECK =====
    // If we're showing filtered results, don't reload - just navigate to the page
    if (isFilteredView) {
        Serial.println("Already showing filtered data, navigating to page");
        if (objects.data_log_page) {
            lv_scr_load(objects.data_log_page);
        }
        return;
    }
    ...
    int startIdx = (totalLines > 20) ? (totalLines - 10) : 0;
   ...
   int topOffset = 80;  // Change this value to move entries up or down
    ...
    lv_label_set_long_mode(objects.sd_data_logging, LV_LABEL_LONG_WRAP);
    lv_obj_set_width(objects.sd_data_logging, lv_pct(100));  // 48% width for left column
...
void processCommand(char* cmd) {
  trimCommand(cmd); // remove leading/trailing spaces

  if (strncmp(cmd, "SETTIME", 7) == 0) {
    int hh, mm, ss, d, m, y;
    if (sscanf(cmd + 7, "%d %d %d %d %d %d", &hh, &mm, &ss, &d, &m, &y) == 6) {
      rtc.adjust(DateTime(y, m, d, hh, mm, ss));
      Serial.println("Time Updated!");
    } else {
      Serial.println("Invalid SETTIME format!");
    }
  } else {
...
// ===== TIMESTAMP FILTER FUNCTIONS =====

// Function to validate timestamp format (YYYY-MM-DD HH:MM:SS)
...
// Function to filter and display log entries by timestamp
void filterLogByTimestamp(const String& startTS, const String& endTS) {
    Serial.println("\n\n========================================");
    Serial.println("=== FILTER FUNCTION CALLED ===");
    Serial.println("========================================");
    ...
    Serial.println(endTS);
    Serial.println();
    
    File file = SD.open("/jar_settings.txt", FILE_READ);
    if (!file) {
        Serial.println("ERROR: Cannot open /jar_settings.txt");
        if (objects.sd_data_logging) {
            lv_label_set_text(objects.sd_data_logging, "Error: jar_settings.txt not found");
        }
        return;
    ...
    Serial.println("--- Starting line-by-line scan ---\n");
    ...
    while (file.available()) {
        ...
        // Parse line format: Jar1,YYYY-MM-DD HH:MM:SS,RPM50,Duration600
        int c1 = line.indexOf(',');
        int c2 = line.indexOf(',', c1 + 1);
        
        if (c1 < 0 || c2 < 0) {
            Serial.println("  --> Invalid format (missing commas)");
            continue;
        }
        ...
        Serial.println("]");
        Serial.print("  Length: ");
        Serial.println(ts.length());
        
        // Check if timestamp is within range
        bool isAfterStart = (ts >= startTS);
        bool isBeforeEnd = (ts <= endTS);
        
        Serial.print("  ts >= startTS? ");
        Serial.println(isAfterStart ? "YES" : "NO");
        Serial.print("  ts <= endTS? ");
        Serial.println(isBeforeEnd ? "YES" : "NO");
        
        if (isAfterStart && isBeforeEnd) {
            Serial.println("  --> ✓ MATCH! Adding to results");
            strcat(buffer, line.c_str());
            strcat(buffer, "\n");
            matchCount++;
            
            // Prevent buffer overflow
            if (strlen(buffer) > 1900) {
                Serial.println("\n!!! Buffer full, stopping scan !!!");
                break;
            }
        } else {
    ...
    Serial.print(" matches out of ");
    Serial.print(totalLines);
    Serial.println(" total lines");
    Serial.println("========================================\n");
    
    // Set flag to indicate filtered view
    isFilteredView = true;
    
    // Display results
    if (matchCount > 0) {
        Serial.println("Displaying results on screen...");
        if (objects.sd_data_logging) {
            char header[256];
            sprintf(header, "=== Filtered Results ===\n%d of %d entries\n\nFrom: %s\nTo: %s\n\n%s", 
                    matchCount, totalLines, startTS.c_str(), endTS.c_str(), buffer);
            lv_label_set_text(objects.sd_data_logging, header);
            configureSdDataLogUI();
        }
        
        // Navigate to the screen
        if (objects.sd_data_logging) {
            lv_obj_t *parent_screen = lv_obj_get_screen(objects.sd_data_logging);
            if (parent_screen) {
                lv_scr_load(parent_screen);
                Serial.println("Navigated to SD data log page");
            } else {
                Serial.println("ERROR: Could not find parent screen");
            }
        }
    } else {
        Serial.println("No matches found - displaying message");
        if (objects.sd_data_logging) {
            char msg[512];
            sprintf(msg, "No entries found\n\nScanned: %d lines\n\nFilter range:\nFrom: %s\nTo: %s\n\nTip for full day search:\nStart: YYYY-MM-DD 00:00:00\nEnd: YYYY-MM-DD 23:59:59", 
                    totalLines, startTS.c_str(), endTS.c_str());
            lv_label_set_text(objects.sd_data_logging, msg);
            configureSdDataLogUI();
        }
    ...
    Serial.println("Filter function complete\n");
}

// Callback for timestamp filter button
void timestampFilterButton_cb(lv_event_t *e) {
    resetActivityTimer();
    ...
    Serial.println("\n\n========================================");
    Serial.println("=== TIMESTAMP FILTER BUTTON CLICKED ===");
    Serial.println("========================================\n");
    
    // Create input dialog for start timestamp
    createTimestampInputDialog(true); // true = start timestamp
}

// Function to create timestamp input dialog
void createTimestampInputDialog(bool isStartTime) {
    ...
    lv_label_set_text(title, isStartTime ? "Enter START Timestamp" : "Enter END Timestamp");
    ...
    DialogData *data = (DialogData*)malloc(sizeof(DialogData));
    data->bg = bg;
    data->ta = ta;
    data->isStart = isStartTime;
    ...
    lv_obj_add_event_cb(kb, [](lv_event_t *e) {
        ...
        if (strcmp(txt, LV_SYMBOL_BACKSPACE) == 0) {
            ...
            // Get the dialog data from parent hierarchy
            lv_obj_t *dialog = lv_obj_get_parent(kb);
            lv_obj_t *bg = lv_obj_get_parent(dialog);
            
            String input = String(lv_textarea_get_text(ta));
            input.trim();
            
            // Validate timestamp
            if (!isValidTimestamp(input)) {
                Serial.println("Invalid timestamp format!");
                lv_obj_t *hint = lv_obj_get_child(dialog, 1); // Get hint label
                lv_label_set_text(hint, "Invalid format! Try again");
                lv_obj_set_style_text_color(hint, lv_palette_main(LV_PALETTE_RED), 0);
                return;
            }
            
            // Determine if this is start or end timestamp
            // Check the title to know which one
            lv_obj_t *title = lv_obj_get_child(dialog, 0);
            const char *title_text = lv_label_get_text(title);
            bool isStart = (strstr(title_text, "START") != NULL);
            
            if (isStart) {
                filterStartTS = input;
                Serial.printf("START timestamp set: %s\n", filterStartTS.c_str());
                
                // Close this dialog
                lv_obj_del(bg);
                
                // Open END timestamp dialog
                createTimestampInputDialog(false);
            } else {
                filterEndTS = input;
                Serial.printf("END timestamp set: %s\n", filterEndTS.c_str());
                
                // Close dialog
                lv_obj_del(bg);
                
                // Perform filtering
                filterLogByTimestamp(filterStartTS, filterEndTS);
            }
        } else {
            lv_textarea_add_text(ta, txt);
        }
    }, LV_EVENT_VALUE_CHANGED, ta);
    ...
    lv_obj_add_event_cb(cancel_btn, [](lv_event_t *e) {
        ...
        Serial.println("Timestamp filter cancelled");
...
/* Register RTC date/time button
if (objects.rtc_button) {
    lv_obj_add_event_cb(objects.rtc_button, RTCManager::buttonCallback, LV_EVENT_CLICKED, NULL);
    Serial.println("RTC button registered");
}*/


// Register RTC date/time button - MAKE SURE THIS LINE EXISTS AND EXECUTES
if (objects.rtc_button) {
    lv_obj_add_event_cb(objects.rtc_button, RTCManager::buttonCallback, LV_EVENT_CLICKED, NULL);
    //Serial.println("RTC button callback registered");
} else {
    //Serial.println("WARNING: rtc_button is NULL - check UI initialization!");
}
...
// Create timestamp filter button below the datetime button
lv_obj_t *filter_button = lv_btn_create(objects.settings_page);  // Use the same page as datetime button
lv_obj_set_size(filter_button, 200, 50);
// Position it below the datetime button
lv_obj_align_to(filter_button, objects.rtc_button, LV_ALIGN_OUT_BOTTOM_MID, 0, 10);

lv_obj_t *filter_label = lv_label_create(filter_button);
lv_label_set_text(filter_label, "Filter by Date");
lv_obj_center(filter_label);

// Register callback
lv_obj_add_event_cb(filter_button, timestampFilterButton_cb, LV_EVENT_CLICKED, NULL);
//Serial.println("Timestamp filter button created and callback registered");


// Create "Show All" button - UPDATE THIS CODE
lv_obj_t *show_all_button = lv_btn_create(objects.settings_page);
lv_obj_set_size(show_all_button, 200, 50);
lv_obj_align_to(show_all_button, filter_button, LV_ALIGN_OUT_BOTTOM_MID, 0, 10);

lv_obj_t *show_all_label = lv_label_create(show_all_button);
lv_label_set_text(show_all_label, "Show All Data");
lv_obj_center(show_all_label);

// Fixed callback with proper screen navigation
lv_obj_add_event_cb(show_all_button, [](lv_event_t *e) {
    if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
        isFilteredView = false;  // Clear filter flag
        filterStartTS = "";
        filterEndTS = "";
        Serial.println("Filter cleared, showing all data");
        
        // Manually trigger the normal SD log display
        if (objects.sd_data_logging) {
            // Get the parent screen
            lv_obj_t *parent_screen = lv_obj_get_screen(objects.sd_data_logging);
            
            // Clear the label and reload all data
            lv_label_set_text(objects.sd_data_logging, "Loading all data...");
            ...
            // Create a fake event to trigger showSdLogOnSettings
            lv_event_t fake_event;
            fake_event.code = LV_EVENT_CLICKED;
            showSdLogOnSettings(&fake_event);
        }
    }
}, LV_EVENT_CLICKED, NULL);

//Serial.println("Show All button created");
...
if (objects.time_home_page) {
    rtcManager.bindUI(objects.time_home_page, objects.rtc_button);
    //Serial.println("RTC UI bound to time_home_page");
} else {
    //Serial.println("WARNING: time_home_page is NULL!");
}


// Initialize RTC
if (rtcManager.begin(21, 22)) {  // Adjust I2C pins if needed
    rtcManager.setErrorManager(&errorManager);
    //Serial.println("RTC initialized, updating home page...");
    
    // Force initial update
    rtcManager.updateHomePageTime();
    
    // Verify the update
    DateTime now = rtcManager.getCurrentTime();
    Serial.printf("Current time should be: %02d:%02d:%02d\n", 
